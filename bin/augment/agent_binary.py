...
span_extractor_prompt = PromptTemplate(
    ...
    template=(
        ...
        "Given the following text, identify and extract all spans that exhibit propagandistic tendencies. Also return a confidence score for each prediction. If no spans are detected, return empty list [] directly."
        "Return each span as a list item.\n\n"
        "Input: {text}\n\n"
        "Return your response in a list of tuples as follows:\n"
        "[('<span_text1>', <Confidence score>), ('<span_text2>', <Confidence score>), etc.]\n\n"
    )
)
...
verifier_prompt = PromptTemplate(
    ...
    template=(
        ...
        "Return your response in a list as follows:\n"
        "['<span_text1>', '<span_text2>', etc]\n\n"
    )
)
...
tools = [span_extractor_tool, verifier_tool]
...
def propaganda_detection_workflow(text):
    ...
    prompt=(
        "You are an expert verifier for propaganda detection. "
        "You are provided with text spans that have propagandistic tendencies. Each span propaganda prediction is associated with a confidence score." 
        "Return a revised version with any corrections, or confirm that all classifications are correct.\n\n"
        "Return the spans with propaganda in a list:\n"
        "['<span_text1>', '<span_text2>', etc]\n\n"
    )
    verified_result = coordinator_agent.run(prompt + f" {span_extraction_result}")
        ...
        try:
            ...
            if len(response) == 0:
                ...
                try:
                    ...
                    preds = [s.replace("[","").replace("(","").replace("]","").replace(")","") for s in response.split(",")]

                new_preds = []
                for l in preds:
                    tec = "propaganda"
                    start,end = find_span(l,sentence)

                    new_preds.append({'text':l,'technique':tec,'start':start,'end':end})
...
if __name__ == "__main__":
    ...
    annots_fname = base_dir + "data/arabic/bin_span/ArMPro_bin_span_test_V2_100.jsonl"
    out_fname = base_dir + "llm_output/preds_TSD=ArMPro_bin_span_test_V2_2steps_agent_gpt4o_out_100.jsonl"
    err_output_file = base_dir + "llm_output/preds_TSD=ArMPro_bin_span_test_V2_2steps_agent_gpt4o_err_100.jsonl"
