        ...
        for line in json_file:
            ...
            if 'paragraph_id' in result:
                if "page_id" in result:
                    if "EN_" in result['page_id']:
                        id = result["page_id"]+str(result["paragraph_id"])
                    else:
...
def api_call(pars, output_file, err_output_file, api_url, model, headers, base_prompt, no_rows):
    k = 0
    for parid, row in pars.items():
        ...
        input_prompt = base_prompt + "News piece: " + sentence + "\n" + \
...
def run_gpt_zero_shot(annots_fname, out_fname, err_output_file, api_url, model, headers, no_rows):
    prompt_text2 = (
    ...
    extra2 =  (f"The potential propaganda techniques that might appear in the text spans: 'Appeal to Time' , 'Conversation Killer' , 'Slogans' , 'Red Herring' , 'Straw Man' , 'Whataboutism' , "
    ...
    prompt_text = prompt_text2 + extra2
    ...
    api_call(filtering_data, output_file, err_output_file, api_url, model, headers, prompt_text, no_rows)
...
if __name__ == "__main__":
    ...
    print(f"{model} {openai_api_base} {openai_api_version}",flush=True)
    ...
    print("Running %d lines.."%l,flush=True)

    if setup == "zshot":
        run_gpt_zero_shot(input_file, options.out_fname, err_output_file, api_url, model, headers, l)