...
def compute_aggregate_scores(all_labels, all_predictions, all_classes):
    ...
    confusion_matrix = compute_confusion_matrix(
        all_labels, all_predictions, labels=all_classes
    )
    ...
    # prf_pos_label = precision_recall_fscore_support(
    #         all_labels, all_predictions, labels=all_classes, pos_label='Yes', average='binary'
    #     )[:-1]
...
# Function to evaluate a model on a given fold
def evaluate_model_svm(
    model, fold, predict_func, exp_dir, tfidfvec, all_classes, **kwargs
):
    instances = [instance for instance, _ in fold]
    labels = [label for _, label in fold]

    predictions, probabilities = predict_func(
        model, instances, exp_dir, tfidfvec, **kwargs
    )
...
def run_model(exp_dir, folds, train_func, predict_func, **kwargs):
    ...
    for fold_idx in range(num_folds):
        ...
        model = train_func(
            folds[fold_idx][0], folds[fold_idx][1], exp_dir, fold_idx, **kwargs
        )
        models.append(model)
        ...
        all_train_evaluations.append(
            ([label for _, label in folds[fold_idx][0]], train_predictions)
        )
        all_dev_evaluations.append(
            ([label for _, label in folds[fold_idx][1]], dev_predictions)
        )
        all_test_evaluations.append(
            ([label for _, label in folds[fold_idx][2]], test_predictions)
        )
    ...
    print("=================================")
    overall_train_results = compute_aggregate_scores(all_train_evaluations, all_classes)
    overall_dev_results = compute_aggregate_scores(all_dev_evaluations, all_classes)
    overall_test_results = compute_aggregate_scores(all_test_evaluations, all_classes)
...
def run_model_bert_vec(
    exp_dir, folds, folds_file_list, train_func, predict_func, **kwargs
):
    # print("Preparing model")
    num_folds = len(folds)
    exp_dir, tmp_dir, models_dir, evaluation_dir = get_experiment_dirs(exp_dir)
    ...
    for fold_idx in range(num_folds):
        print("Training fold %d" % (fold_idx))
        model = train_func(
            folds_file_list[fold_idx][0],
            folds_file_list[fold_idx][1],
            exp_dir,
            fold_idx,
            **kwargs
        )
